...
import (
	...
	"github.com/ethosure/coding_ethos/go/internal/managedcapture"
...
case "$1 $2" in
  "list -buildvcs=false")
    printf 'github.com/ethosure/coding_ethos/go/pkg\n'
    printf 'github.com/ethosure/coding_ethos/go/internal/e2e\n'
    exit 0
    ;;
  "test -p=1")
    exit 0
		...
		Evaluators: []policy.Evaluator{{
			...
			Options: map[string]any{
				...
				"when": `coverage.exists(item,
					...
					item.percent < 90.0 &&
					(item.package == "" ||
					item.package.startsWith("github.com/ethosure/coding_ethos/go")))`,
			},
		}},
		PrincipleIDs: []string{
			"testing-as-specification",
