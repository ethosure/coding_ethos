...
func TestParseGoTestPackageFailureWithoutTestOutput(t *testing.T) {
	...
	parsed := diagnostics.Parse(
		"go-test",
		`{"Action":"fail","Package":"github.com/ethosure/coding_ethos/go/cmd/tool","Elapsed":0.013}`,
		"",
	)
	...
	assertDiagnostic(t, parsed, diagnostics.Diagnostic{
		...
		Severity: "error",
		Code:     "package_failed",
		Message:  "Go test package failed: github.com/ethosure/coding_ethos/go/cmd/tool",
	})
}
...
func TestParseGoTestCoverageDiagnostics(t *testing.T) {
	...
	parsed := diagnostics.Parse(
		"go-test",
		`{"Action":"output","Package":"github.com/ethosure/coding_ethos/go/pkg",`+
	...
	assertDiagnosticAt(t, parsed, 0, diagnostics.Diagnostic{
		Metadata: map[string]any{
			...
			"package":          "github.com/ethosure/coding_ethos/go/pkg",
		},
		Tool:     "go-test",
		Severity: "record",
		Code:     "coverage-package",
		Message:  "Go test coverage for github.com/ethosure/coding_ethos/go/pkg is 82.40%.",
	})
	assertDiagnosticAt(t, parsed, 1, diagnostics.Diagnostic{
		Metadata: map[string]any{
...
func goToolchainDiagnosticCases() []toolchainDiagnosticCase {
	return []toolchainDiagnosticCase{
		...
		},
		{
			name: "go-vet-text",
			tool: "go-vet",
			output: "# github.com/ethosure/coding_ethos/go/pkg\n" +
...
const goTestFixture = `{"Action":"run","Package":"github.com/ethosure/coding_ethos/` +
	`go/cmd/coding-ethos-policy","Test":"TestPolicyUsage"}` + "\n" +
	`{"Action":"output","Package":"github.com/ethosure/coding_ethos/` +
	`go/cmd/coding-ethos-policy","Test":"TestPolicyUsage",` +
	...
	`second failure\n"}` + "\n" +
	`{"Action":"fail","Package":"github.com/ethosure/coding_ethos/` +
	`go/cmd/coding-ethos-policy","Test":"TestPolicyUsage","Elapsed":0}` + "\n" +
	`{"Action":"pass","Package":"github.com/ethosure/coding_ethos/go/internal/hooks",` +
	`"Elapsed":0.187}`
