...
- CLA Assistant contribution certification in `CONTRIBUTING.md`; the PR
  ...
  CODEOWNERS names `@paudley` and `@ErinAudley` as Ethosure Governance Inc.
  project owners.
- Governance and continuity are documented in `CONTRIBUTING.md`; Ethosure
  Governance Inc. maintains the project, and the two CODEOWNERS-listed
...
`https://github.com/ethosure/coding_ethos` as its repository URL, wait for the
